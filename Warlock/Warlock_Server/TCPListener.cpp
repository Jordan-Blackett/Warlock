...
TCPListener::TCPListener(std::string ipAddress, int port, MessageRecievedHandler handler)
	: ipAddress_(ipAddress), port_(port), MessageRecieved(handler)
{
}
	...
	while (true)
	{
		...
		for (int i = 0; i < connections_.size(); i++)
		{
			...
			if (FD_ISSET(connections_[i].socket_, &readfds_))
			{
				...
				//connections_[i].incomingPacketLength = 2;

				ZeroMemory(buf, MAX_BUFFER_SIZE);
				//int bytesReceived = recv(connections_[i].socket_, buf + connections_[i].bytesReceived, connections_[i].incomingPacketLength - connections_[i].bytesReceived, 0);
				...
				if (bytesReceived > 0)
				{
					connections_[i].incomingPacketLength = 8; //value of first 16 bytes
					...
					std::cout << "connections_ bytesReceived " << connections_[i].bytesReceived << std::endl;
					if (connections_[i].bytesReceived == connections_[i].incomingPacketLength 
						&& MessageRecieved != NULL)
					{
						std::cout << std::string(connections_[i].buffer, 0, connections_[i].incomingPacketLength) << std::endl;
