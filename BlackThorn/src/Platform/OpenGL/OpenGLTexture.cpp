...
namespace BlackThorn {
	...
	OpenGLTexture2D::OpenGLTexture2D(const std::string& path, const int tilteWidth, const int tilteHeight)
	{
		...
		GLsizei tile_w = tilteWidth, tile_h = tilteHeight;
		GLsizei tiles_x = (m_Width / tile_w);
		GLsizei	tiles_y = (m_Height / tile_h);
		GLsizei	tile_count = tiles_x * tiles_y;

		BT_CORE_ASSERT(internalFormat & dataFormat, "Format not supported.");

		glCreateTextures(GL_TEXTURE_2D_ARRAY, 1, &m_RendererID);
		glTextureStorage3D(m_RendererID, 1, internalFormat, tile_w, tile_h, tile_count);

		{
			...
			for (GLsizei i = 0; i < tile_count; ++i)
			{
				...
				glCopyImageSubData(temp_tex, GL_TEXTURE_2D, 0, x, y, 0, m_RendererID, GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, tile_w, tile_h, 1);
			}
